...
// macOS, Linux and Windows jobs, so they are compiled out of the WebAssembly build.
    ...
    /// Property-based recovery differential for the JSON grammar.
    ///
    ...
    @Suite("JSON recovery fuzz differential")
    struct JSONRecoveryFuzzTests {
        ...
        func recoveryMatchesReference() throws {
            ...
            let report = RecoveryFuzz.compare(seeds: Corpus.json, reference: reference, allStar: allStar)
            #expect(report.divergences.isEmpty, "\(report.checked) inputs checked; \(report.summary)")
        }
    }

    /// Deterministic recovery-differential fuzzing, shared in spirit with the Lua and C structural suites.
    ///
